...
gpu_uuid=$(nvidia-smi -i "$CUDA_VISIBLE_DEVICES" --query-gpu=uuid --format=csv,noheader,nounits)
...
for format in "${formats[@]}"; do
    ...
    node_dir="${tmp_dir}/matrix-vector-output/${node_id}/${gpu_id}/perlmutter_${data_dir}"
    if [ ! -d "$node_dir" ]; then
        mkdir -p "$node_dir"
    ...
    # Define output file to capture the "main" binary output
    output_file="${node_dir}/${gpu_id}.csv"

    # Run the "main" binary and capture its output into gpu_id.csv
    "${src_dir}/main" 32768 100 20 0 > "$output_file"
...
dest_node_dir="${script_dir}/perlmutter-output/matrix-vector-output/${node_id}"
...
cp -r "$tmp_dir/matrix-vector-output/${node_id}/${gpu_id}" "$dest_gpu_dir"
