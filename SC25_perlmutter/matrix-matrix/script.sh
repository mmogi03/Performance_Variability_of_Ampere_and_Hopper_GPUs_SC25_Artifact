...
for format in "${formats[@]}"; do
    ...
    node_dir="${tmp_dir}/matrix-matrix-output/${node_id}/${gpu_id}/perlmutter_${data_dir}"
    if [ ! -d "$node_dir" ]; then
        mkdir -p "$node_dir"
...
dest_node_dir="${script_dir}/perlmutter-output/matrix-matrix-output/${node_id}"
...
cp -r "$tmp_dir/matrix-matrix-output/${node_id}/${gpu_id}" "$dest_gpu_dir"
