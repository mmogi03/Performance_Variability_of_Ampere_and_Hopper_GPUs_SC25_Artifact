...
for format in "${formats[@]}"; do
    ...
    node_dir="${tmp_dir}/matrix-matrix-cuda-output/${node_id}/${gpu_id}/perlmutter_${data_dir}"
    if [ ! -d "$node_dir" ]; then
        mkdir -p "$node_dir"
    ...
    "${src_dir}/main" 32768 100 20 0 0 > "$output_file"
...
dest_node_dir="${script_dir}/perlmutter-output/matrix-matrix-cuda-output/${node_id}"
...
cp -r "$tmp_dir/matrix-matrix-cuda-output/${node_id}/${gpu_id}" "$dest_gpu_dir"
