...
cd "${script_dir}/matrix-matrix-fourhour"
formats=("BF16" "FP64")
...
for format in "${formats[@]}"; do
    ...
    node_dir="${tmp_dir}/matrix-matrix-fourhour-output/${node_id}/${gpu_id}/vista_${data_dir}"
    if [ ! -d "$node_dir" ]; then
        mkdir -p "$node_dir"
    ...
    if [ "$format" = "BF16" ]; then
        # Parameters for BF16
        "${src_dir}/main" 32768 0 0 1 > "$output_file"
...
dest_node_dir="${script_dir}/vista-output/matrix-matrix-fourhour-output/${node_id}"
...
cp -r "$tmp_dir/matrix-matrix-fourhour-output/${node_id}/${gpu_id}" "$dest_gpu_dir"
