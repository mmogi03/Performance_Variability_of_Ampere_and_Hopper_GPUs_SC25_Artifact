...
// and converts the result to __nv_bfloat16.
...
void runGemvExperiment(int dimension, int repeats, int warmUpPeriod)
{
    ...
    int numElementsVector = m;  // assuming square matrices and vectors of length m
    ...
    // Allocate unified memory for matrix A and vectors x and y (BF16)
    __nv_bfloat16 *d_A, *d_x, *d_y;
    checkCuda(cudaMallocManaged(&d_A, numElementsMatrix * sizeof(__nv_bfloat16)));
    checkCuda(cudaMallocManaged(&d_x, numElementsVector * sizeof(__nv_bfloat16)));
    checkCuda(cudaMallocManaged(&d_y, numElementsVector * sizeof(__nv_bfloat16)));

    // Allocate unified memory for arrays of pointers (for batched operation, batchCount = 1)
    __nv_bfloat16 **d_Aarray, **d_xarray, **d_yarray;
    checkCuda(cudaMallocManaged(&d_Aarray, sizeof(__nv_bfloat16*)));
    checkCuda(cudaMallocManaged(&d_xarray, sizeof(__nv_bfloat16*)));
    checkCuda(cudaMallocManaged(&d_yarray, sizeof(__nv_bfloat16*)));
    ...
    *d_yarray = d_y;
    ...
    GPU_fill_rand_bf16(d_A, numElementsMatrix, 0ULL, 0.0f, 1.0f);
    GPU_fill_rand_bf16(d_x, numElementsVector, 0ULL, 0.0f, 1.0f);
    // Initialize y to zeros
    for (int i = 0; i < numElementsVector; i++)
    {
        d_y[i] = __float2bfloat16(0.0f);
    }
    ...
    // Print CSV header with columns: dimension,iteration,start_ts,stop_ts,time(ms)
    cout << "dimension,iteration,start_ts,stop_ts,time(ms)" << endl;
    ...
    // Create timing events for GEMV iterations.
    ...
    // Iteration loop: each iteration performs the batched GEMV operation.
    // All iterations, including warm-up iterations, are logged.
    for (int rep = -warmUpPeriod; rep < repeats; rep++)
    {
        ...
        // GEMV: y = alpha * A * x
        checkCublas(cublasTSTgemvBatched(handle,
            CUBLAS_OP_N, m, n,
            alpha,
            (const __nv_bfloat16 *const *)d_Aarray, lda,
            (const __nv_bfloat16 *const *)d_xarray, incx,
            beta,
            (__nv_bfloat16 *const *)d_yarray, incy,
            batchCount));
