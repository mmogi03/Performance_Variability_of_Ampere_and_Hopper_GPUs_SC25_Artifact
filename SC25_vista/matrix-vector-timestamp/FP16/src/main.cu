...
void runGemvExperiment(int dimension, int repeats, int warmUpPeriod)
{
    ...
    *d_yarray = d_y;
    ...
    // Initialize data arrays with random values in [0.9, 1.0]
    GPU_fill_rand_fp16(d_A, numElementsMatrix, 0ULL, 0.9f, 1.0f);
    GPU_fill_rand_fp16(d_x, numElementsVector, 0ULL, 0.9f, 1.0f);
