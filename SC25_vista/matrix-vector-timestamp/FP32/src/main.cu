...
void runGemvExperiment(int dimension, int repeats, int warmUpPeriod)
{
    ...
    // Allocate unified memory for matrix A and vectors x and y (FP32)
    float *d_A, *d_x, *d_y;
    checkCuda(cudaMallocManaged(&d_A, numElementsMatrix * sizeof(float)));
    checkCuda(cudaMallocManaged(&d_x, numElementsVector * sizeof(float)));
    checkCuda(cudaMallocManaged(&d_y, numElementsVector * sizeof(float)));

    // Allocate unified memory for arrays of pointers (for batched operation, batchCount = 1)
    float **d_Aarray, **d_xarray, **d_yarray;
    checkCuda(cudaMallocManaged(&d_Aarray, sizeof(float*)));
    checkCuda(cudaMallocManaged(&d_xarray, sizeof(float*)));
    ...
    *d_yarray = d_y;
    ...
    // Initialize data arrays with random values in [0.9, 1.1]
    GPU_fill_rand_fp32(d_A, numElementsMatrix, 0ULL, 0.9f, 1.1f);
    GPU_fill_rand_fp32(d_x, numElementsVector, 0ULL, 0.9f, 1.1f);
    ...
    for (int rep = -warmUpPeriod; rep < repeats; rep++)
    {
        ...
        // GEMV: y = alpha * A * x
        checkCublas(cublasSgemvBatched(handle,
            CUBLAS_OP_N, m, n,
            alpha,
            (const float *const *)d_Aarray, lda,
            (const float *const *)d_xarray, incx,
            beta,
            (float *const *)d_yarray, incy,
            batchCount));
