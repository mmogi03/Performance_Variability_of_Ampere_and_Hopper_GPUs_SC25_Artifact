...
void runScalarExperiment(int size, int repeats, int warmUpPeriod)
{
    ...
    // Initialize vector: values in [0.9, 1.1].
    ...
    // Create timing events for scalar iterations.
    ...
    // Scalar constants for two operations.
    // First scaling: multiply by 2.0, second scaling: multiply by 0.5.
    float alpha = 2.0f;
    float beta  = 0.5f;
    int incx = 1;

    // Iteration loop: each iteration performs two scalar scaling operations.
